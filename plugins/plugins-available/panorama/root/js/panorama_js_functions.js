...
var TP = {
        ...
        Ext.Ajax.request({
            ...
            callback: function(options, success, response) {
                ...
                if(!success) {
                    ...
                    /* update all maps */
                    for(var map in ref.maps) {
                        for(var x=0; x<ref.maps[map].length; x++) {
                            ref.maps[map][x].refreshHandler();
                        }
                        delete ref.maps[map];
                    }
    ...
    getStatusReq: function(tab, id, xdata) {
        ...
        var ref      = { filter: {}, hosts: {}, hostgroups: {}, services: {}, servicegroups: {}, sites: {}, maps: {} };
        var count  = 0;
        for(var nr=0; nr<panels.length; nr++) {
            var p = panels[nr];
            ...
            if(p.xdata && p.xdata.general) {
                ...
                /* update maps */
                else if(p.xdata.general.map) {
                    if(ref.maps[p.xdata.general.map] == undefined) { ref.maps[p.xdata.general.map] = []; }
                    ref.maps[p.xdata.general.map].push(p);
                    count++;
                }
            }
    ...
    getTabState: function(tab_id, callback) {
        ...
        var group = { services: {         ok: 0,          warning: 0,          critical: 0,          unknown: 0,       pending: 0,
        ...
        var res = TP.get_group_status({ group: group, incl_svc: true, incl_hst: true, incl_ack: true, incl_downtimes: true});
